...
import { historyPostApi } from '../../lib/postAxios';
...
export default function ChatTableScore({ selectNum, setSelectNum, beforeSelectNum, setBeforeSelectNum, nabiNum, beforeNabiNum, setBeforeNabiNum, setAllPageNum }: tableScoreType) {
    ...
    const getHistoryAll = useCallback(
        async (nickname: string, beforNums: number, nums: number, firstHistory: number, lastHistory: number) => {
            try {
                if (nabiNum !== 0) return;
                ...
                const res = await historyPostApi.getRightHistory(nickname, beforNums, nums, firstHistory, lastHistory, "all");
    ...
    const getHistoryNomal = useCallback(
        async (nickname: string, beforNums: number, nums: number, firstHistory: number, lastHistory: number) => {
            try {
                ...
                const res = await historyPostApi.getRightHistory(nickname, beforNums, nums, firstHistory, lastHistory, "normal");
                setTableList(res.data[0]);
                const totals = (parseInt(res.data[1].total) % 15 !== 0 || Math.floor(parseInt(res.data[1].total) / 15) === 0 ? Math.floor(parseInt(res.data[1].total) / 15) + 1
                    : Math.floor(parseInt(res.data[1].total) / 15));
                setAllPageNum(totals);
    ...
    const getHistoryLadder = useCallback(
        async (nickname: string, beforNums: number, nums: number, firstHistory: number, lastHistory: number) => {
            try {
                ...
                const res = await historyPostApi.getRightHistory(nickname, beforNums, nums, firstHistory, lastHistory, "ladder");
                setTableList(res.data[0]);
                const totals = (parseInt(res.data[1].total) % 15 !== 0 || Math.floor(parseInt(res.data[1].total) / 15) === 0 ? Math.floor(parseInt(res.data[1].total) / 15) + 1
                    : Math.floor(parseInt(res.data[1].total) / 15));
                setAllPageNum(totals);
    ...
    useEffect(() => {
        if (nabiNum === 0) getHistoryAll(eventInfo?.target || "", 1, 1, 0, 0);
        if (nabiNum === 1) getHistoryNomal(eventInfo?.target || "", 1, 1, 0, 0);
        if (nabiNum === 2) getHistoryLadder(eventInfo?.target || "", 1, 1, 0, 0);
    }, [nabiNum, getHistoryAll, getHistoryNomal, getHistoryLadder, eventInfo?.target])
    ...
    useEffect(() => {
        ...
        if (beforeSelectNum !== selectNum) {
            if (nabiNum === 0) getHistoryAll(eventInfo?.target || "", beforeSelectNum, selectNum, firstHistoryPk, LastHistoryPk);
            if (nabiNum === 1) getHistoryNomal(eventInfo?.target || "", beforeSelectNum, selectNum, firstHistoryPk, LastHistoryPk);
            if (nabiNum === 2) getHistoryLadder(eventInfo?.target || "", beforeSelectNum, selectNum, firstHistoryPk, LastHistoryPk);
        }
    }, [selectNum, beforeSelectNum, beforeNabiNum, setBeforeNabiNum, nabiNum, getHistoryAll, getHistoryNomal, getHistoryLadder, eventInfo?.target])
