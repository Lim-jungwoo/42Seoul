...
import style from './MapCustomTwo.module.css';
import { MapInfoType, DisplayType, BrickType, BallType, TrayType, ScoreType, GameOutType, WatchOnePropsType, WatchTwoPropsType } from './MapInterfaces';


const BallDiv = (ballData: BallType) => {
    return (
        <div className={style.mapBlockBall} style={{ top: `${ballData.positionY}px`, left: `${ballData.positionX}px`, width: `${ballData.radius * 2}px`, height: `${ballData.radius * 2}px` }}></div>
    )
}

const BrickDiv = (brickData: BrickType) => {
    return (
        <div className={style.mapBlockBrick} style={{ top: `${brickData.positionY}px`, left: `${brickData.positionX}px`, width: `${brickData.width}px`, height: `${brickData.height}px` }}></div>
    )
}

const CrowdDiv = ({ spectators }: { spectators: number }) => {
    return (
        <div className={style.mapBlockCrowd}>watch {spectators}</div>
    )
}
...
const LeaveBtn = () => {
    ...
    return (
        <button className={style.mapBlockLeave} onClick={leaveMap}>관전 떠나기</button>
    )
}
...
const PlayOneDiv = (trayOneData: TrayType) => {
    return (
        <div className={style.mapBlockTrayPlayOne}
            style={{
...
const PlayTwoDiv = (trayTwoData: TrayType) => {
    return (
        <div className={style.mapBlockrayPlayTwo}
            style={{
...
export default function MapCustomTwo({ mapInfo }: { mapInfo: MapInfoType }) {
    const nabi = useNavigate();
    ...
    const [brickData] = useState<BrickType[]>(mapInfo.brick);
    ...
    }, [nabi, roomnumber, gameOutNabi, keyDownPress, keyUpPress, mapInfo.socket, userType]);
    ...
    return (
        <>
            <div className={style.mapBlockBase}>
                <div className={style.mapBlockCenter} style={{ width: `${disPlaysData.width}px`, height: `${disPlaysData.height}px` }}>
                    <div className={style.mapBlockScore}>
                    ...
                    <BrickDiv {...brickData[0]} />
                    <BrickDiv {...brickData[1]} />
                </div>
            </div>
            {loding ? <div className={style.mapBlockLodings}><div className={style.mapBlockLodingsDiv}>Ready {counters}</div></div> : null}
            {results ?
                <div className={style.mapBlockLodings}>
                    <div className={`${style.mapBlockLodingsDiv} ${style.mapBlockWinDiv}`}>Winner</div>
                    <div className={style.mapBlockLodingsDiv}>{gameOutData.winner}</div>
                    <div className={style.mapBlockLodingsDiv}>{gameOutData.scoreresult}</div>
                </div> : null}
